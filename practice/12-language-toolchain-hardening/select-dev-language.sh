...
# Installs one mise-managed tool per selection in OMAWSL_LANGUAGES. Nothing
# is pre-selected by default and selecting nothing is a valid, expected
# state - each branch below no-ops cleanly if its option wasn't picked.
#
# TODO(this lesson): Elixir's own compiler is written in Erlang, and mise's
# elixir plugin does not pull Erlang in for you - erlang must already be
# mise-installed and on PATH before elixir's own post-install step runs, or
# it fails looking for `erl`. Make the Elixir branch below install erlang
# first, in the same call (not backgrounded or parallel). Every other
# branch should keep behaving exactly as it already does.
omawsl_select_dev_language() {
