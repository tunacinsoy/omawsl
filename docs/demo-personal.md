...
- [ ] **Font: the enhanced Nerd Font** (not the zero-install fallback) — full icon-glyph
      rendering wherever a tool requests one, fits the "full power-user tour" framing. See
      `windows/fonts/README.md` for the exact download (not vendored in this repo) and its font
      family name.
- [ ] No VS Code/Cursor/Docker Desktop installs needed for this file — the personal demo's
      editor picks (opencode, Claude Code CLI) and Docker mode (Engine) need nothing on the
      Windows side at all.
...
- [ ] Deliberately leave one orphan tool (e.g. opencode) one version behind, so step 14 below
...
- [ ] **2. Merge the Windows Terminal JSON fragment.**
      ...
      `windows/windows-terminal.json`'s `profiles.defaults` and `actions` keys into your own
      `settings.json` (don't paste over the whole file).
      Say: "One JSON edit, two fixes at once — sets the Nerd Font as the profile font, and
      unbinds Windows Terminal's own `Alt+Left/Down/Up/Right`, which otherwise steals those
      keystrokes before zellij — the terminal multiplexer omawsl ships — ever sees them."
...
- [ ] **5. Docker pick: Engine (the default).**
...
- [ ] **6. Pick languages: Ruby on Rails + Go.**
